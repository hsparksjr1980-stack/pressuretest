...
def render_financial_model():
    ...
    render_gotcha_section(page="financial_model", title="Economic gotcha moments", max_items=3)
    ...
    with st.expander("1) FDD Investment Range", expanded=not section_done("fdd_investment")):
        st.warning(
            """
            **Be aware before using this range**
            - The FDD low and high investment figures are estimates only
            - They do not reflect your exact local market
            - Real estate, contractors, buildout scope, and inflation can all push these numbers higher
            - These ranges are often already a few years old by the time you are using them
        ...
        render_framework_block(
            "How to Use the FDD Investment Range",
            look_at=[
                "The low and high investment range",
                "What is included versus excluded",
                "Franchise fee treatment",
                "Whether your market has materially higher construction, rent, or contractor costs",
            ],
            common=[
                "FDD investment ranges are often dated by the time you use them",
                "Local contractor, real estate, and inflation pressure can push costs above the printed range",
                "Buyers often underestimate buildout, delays, and working capital needs",
            ],
            ask=[
                "What do local broker, contractor, and vendor quotes suggest instead of the FDD range?",
                "Which categories in this range are most likely understated in my market?",
                "What contingency should I carry if opening slips or bids come in higher?",
            ],
            pressure=[
                "Add a 10–20% buffer and see if the deal still makes sense",
                "Assume opening takes longer and costs more than expected",
                "Do not treat the printed range as a true cap",
            ],
        )
    ...
    with st.expander("2) FDD System P&L", expanded=section_done("fdd_investment") and not section_done("fdd_pl")):
        st.warning(
            """
            **Be aware before using the system P&L**
            - Revenue is for the system as a whole and is only an average
            - COGS is system-wide and may not match your local market
            - Labor varies heavily by geography and wage environment
            - Occupancy should ideally stay around 10% or less of revenue
            - These figures are reference points, not promises

            A strong system average does not mean your store will perform the same way.
            """
        )

        render_framework_block(
            "How to Think About the System P&L",
            look_at=[
                "System revenue",
                "System COGS, labor, and occupancy assumptions",
                "Whether those assumptions transfer to your market",
            ],
            common=[
                "System P&Ls often reflect a blend of stronger and more mature stores",
                "Labor, food cost, and rent can vary materially by geography",
                "Averages can make weak site economics look more acceptable than they are",
            ],
            ask=[
                "Which of these cost lines are likely different in my market?",
                "If labor or COGS run worse locally, do the unit economics still hold up?",
                "Am I treating a system average like a location forecast?",
            ],
            pressure=[
                "Run the model with higher labor and COGS",
                "Keep occupancy at or below roughly 10% and see what rent that implies",
                "Assume your location performs below average until proven otherwise",
            ],
        )
    ...
    with st.expander("3) FDD Revenue Distribution", expanded=section_done("fdd_pl") and not section_done("fdd_distribution")):
        st.warning(
            """
            **Be aware before using revenue distribution**
            - These are still system-level estimates or distributions
            - They are based on reported units, not your exact site
            - This does not dictate how your location will perform
            - Use this to frame reality, not to predict success
            """
        )

        render_framework_block(
            "How to Use Revenue Distribution",
            look_at=[
                "Average, median, top quartile, and bottom quartile system performance",
                "How wide the spread is between strong and weak stores",
            ],
            common=[
                "Distribution data is descriptive, not predictive",
                "One location can fall materially below average even in a good system",
                "Buyers often anchor on the average instead of respecting the spread",
            ],
            ask=[
                "What would have to be true for my store to perform at or above average?",
                "What would drive me toward the lower half of the system instead?",
                "Am I evaluating location-specific economics or borrowing system confidence?",
            ],
            pressure=[
                "Model below-average performance first",
                "See what happens if you land closer to the median or bottom quartile",
                "Treat the spread as a warning about volatility, not as optional context",
            ],
        )
    ...
    with st.expander("4) Financing", expanded=section_done("fdd_distribution") and not section_done("financing")):
        st.warning(
            """
            **Be aware before modeling financing**
            - SBA loans and most startup business loans are personally guaranteed
            - Banks want to know they can recover their money if something goes wrong
            - That can mean your home, savings, and other personal assets are exposed
            - Do not treat this like simple business-only risk
            """
        )
        ...
        render_framework_block(
            ...
            common=[
                "Startup business loans are usually personally guaranteed",
                "Banks expect meaningful owner downside alignment",
                "Many buyers focus on getting approved, not on surviving the payment burden",
            ],
            ask=[
                "What personal assets are really exposed if this fails?",
                "Can I survive if ramp takes much longer than expected?",
                "What happens if the business never reaches the modeled sales level?",
            ],
            pressure=[
                "Model the payment against lower revenue and higher costs",
                "Assume your downside case lasts longer than expected",
                "If the deal only works under ideal conditions, the financing is too aggressive",
            ],
        )
    ...
    with st.expander("6) Startup Assumptions", expanded=section_done("deal_setup") and not section_done("startup")):
        st.warning(
            """
            **Be aware before using startup assumptions**
            - Costs almost always come in higher than you think
            - Delays cost money
            - Buildout, equipment, signage, training, opening costs, and working capital all need buffers
            - Conservative numbers are usually more useful than optimistic ones
            """
        )
    ...
    with st.expander("7) Operating Assumptions", expanded=section_done("startup") and not section_done("operating")):
        st.warning(
            """
            **Be aware before using operating assumptions**
            - Costs are usually higher than expected
            - Ramp is usually slower than expected
            - Rent pressure, labor pressure, and COGS pressure show up fast
            - Use conservative numbers, not best-case numbers
            """
        )

        render_framework_block(
            "How to Build Operating Assumptions",
            look_at=[
                "Average ticket",
                "Transactions per day",
                "Rent and CAM",
                "Labor structure",
                "COGS, marketing, royalty, and leakage",
            ],
            common=[
                "Buyers usually assume faster ramp and lower cost pressure than reality",
                "Rent, labor, and food costs tend to move against the operator, not in their favor",
                "Small misses in ticket count or labor can materially change the outcome",
            ],
            ask=[
                "Which of these inputs are real and which are still guesses?",
                "How much of my revenue assumption depends on ideal traffic or conversion?",
                "If costs run higher and ramp runs slower, does the deal still survive?",
            ],
            pressure=[
                "Use higher costs than you hope for",
                "Use slower ramp and lower traffic than you want",
                "If the model only works under best-case assumptions, it is not ready to trust",
            ],
        )
        ...
        with st.form("form_operating"):
            ...
            if submit_7:
                ...
                st.session_state["fm_starting_cash"] = st.session_state["user_working_cap"]
                st.session_state["fm_merchant_pct"] = merchant_pct / 100
                st.session_state["fm_leakage_pct"] = leakage_pct / 100
    ...
    if all_done:
        ...
        st.markdown("### What to Ask Before You Trust This Model")
        st.write("- Which assumptions here are still based on system averages instead of local evidence?")
        st.write("- Which costs are still placeholders rather than bids or confirmed pricing?")
        st.write("- If revenue ramps slower than expected, how long does my cash really last?")
        st.write("- Which one variable, if wrong, breaks the deal first?")
