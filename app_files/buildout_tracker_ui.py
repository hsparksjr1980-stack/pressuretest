...
def _render_summary_cards(summary: dict) -> None:
    readiness_label = "Ready" if summary["readiness_score"] >= 85 else "In Progress" if summary["readiness_score"] >= 50 else "Blocked"
    blocked_label = "On Track" if summary["blocked_tasks"] == 0 else "Critical" if summary["blocked_tasks"] >= 3 else "Caution"
    overdue_label = "On Track" if summary["overdue_tasks"] == 0 else "Critical" if summary["overdue_tasks"] >= 3 else "Caution"
    variance_label = "Good" if summary["variance_total"] <= 0 else "Critical" if summary["variance_total"] > max(summary["budget_total"] * 0.1, 5000) else "Caution"
    spend_label = "Ready" if summary["actual_total"] > 0 else "In Progress"

    c1, c2, c3, c4 = st.columns(4, gap="medium")
    with c1:
        status_card("Readiness", readiness_label, f"{summary['readiness_score']}% complete across tracked tasks.")
    with c2:
        status_card("Tasks complete", spend_label, f"{summary['complete_tasks']} of {summary['total_tasks']} tasks are complete.")
    with c3:
        status_card("Blocked", blocked_label, f"{summary['blocked_tasks']} tasks are currently blocked.")
    with c4:
        status_card("Overdue", overdue_label, f"{summary['overdue_tasks']} tasks are overdue.")

    st.markdown('<div class="rc-gap-md"></div>', unsafe_allow_html=True)

    c5, c6, c7 = st.columns(3, gap="medium")
    with c5:
        status_card("Budget", "In Progress", f"Budget assigned: ${summary['budget_total']:,.0f}")
    with c6:
        status_card("Actual spend", spend_label, f"Spend recorded: ${summary['actual_total']:,.0f}")
    with c7:
        status_card("Variance", variance_label, f"Actual less budget: ${summary['variance_total']:,.0f}")
...
def _render_risk_callouts(summary: dict) -> None:
    ...
    if len(incomplete_critical_df) > 0:
        items = "".join(f"<li>{task}</li>" for task in incomplete_critical_df["Task"].tolist()[:6])
        st.markdown(
            f"""
            <div class="bt-blocker-card">
                <div class="bt-flag" style="background:#EF4444;"></div>
                <div style="font-size:0.76rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#991B1B;margin-bottom:0.35rem;">
                    Critical path
                </div>
                <div style="font-size:1.18rem;font-weight:800;color:#7F1D1D;margin-bottom:0.28rem;">
                    {len(incomplete_critical_df)} critical launch items still open
                </div>
                <div style="font-size:0.95rem;line-height:1.5;color:#7F1D1D;margin-bottom:0.6rem;">
                    Resolve the highest-impact launch blockers first. Do not treat permits, contractor uncertainty, budget approval, or core systems setup as minor items.
                </div>
                <ul style="margin:0;padding-left:1.1rem;color:#7F1D1D;line-height:1.5;">
                    {items}
                </ul>
            </div>
            """,
    ...
    if len(overdue_df) > 0:
        items = "".join(
            f"<li>{row['Task']} — due {row['Due Date']} ({row['Status']})</li>"
            for _, row in overdue_df[["Task", "Due Date", "Status"]].head(6).iterrows()
        )
        st.markdown(
            f"""
            <div class="bt-warning-card">
                <div class="bt-flag" style="background:#F59E0B;"></div>
                <div style="font-size:0.76rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#92400E;margin-bottom:0.35rem;">
                    Watchouts
                </div>
                <div style="font-size:1.05rem;font-weight:800;color:#92400E;margin-bottom:0.28rem;">
                    {len(overdue_df)} tasks are overdue
                </div>
                <ul style="margin:0;padding-left:1.1rem;color:#92400E;line-height:1.5;">
                    {items}
                </ul>
            </div>
            """,
...
def render_buildout_tracker() -> None:
    ...
    render_section_intro(
        title="Tracker",
        body="Use the editor below as the working source of truth. Save once after editing so the latest values are committed cleanly.",
    )
    st.markdown('<div class="rc-gap-md"></div>', unsafe_allow_html=True)
    st.caption("Tip: edit the cell, then click Save tracker changes once. The form submit commits the latest edit and saves it in the same action.")

    with st.form("buildout_tracker_form"):
        ...
        save_tracker = st.form_submit_button("Save tracker changes", use_container_width=True, type="primary")

    edited_df = _prepare_dataframe(pd.DataFrame(edited_df))
    if save_tracker:
        st.session_state["buildout_tracker_df"] = edited_df.copy()
