...
def build_margin_of_safety(metrics, downside_metrics, sources_uses):
    """
    Returns a simple safety label and explanation so the UI can interpret the model.
    """
    lowest_cash = _safe_float(metrics.get("lowest_cash", 0.0))
    downside_lowest_cash = _safe_float(downside_metrics.get("lowest_cash", 0.0))
    dscr = _safe_float(metrics.get("dscr", 0.0))
    gap = _safe_float(sources_uses.get("gap", 0.0))

    if gap > 0 or lowest_cash < 0 or downside_lowest_cash < 0 or dscr < 1.0:
        return {
            "label": "Thin",
            "summary": "The structure has little room for error. A modest miss in revenue, margin, or timing can create a funding or liquidity problem.",
        }
    if dscr < 1.25 or downside_lowest_cash < 25000:
        return {
            "label": "Moderate",
            "summary": "The deal may work, but there is not a wide cushion. Execution discipline and early performance matter a lot.",
        }
    return {
        "label": "Strong",
        "summary": "The current model shows a healthier cash and coverage cushion than a fragile deal, though the assumptions still need validation.",
    }


def build_what_must_be_true(inputs, metrics, sources_uses):
    annual_revenue = _safe_float(inputs.get("annual_revenue", 0.0))
    labor_pct = _safe_float(inputs.get("labor_pct", 0.0))
    cogs_pct = _safe_float(inputs.get("cogs_pct", 0.0))
    break_even_month = metrics.get("break_even_month")
    dscr = _safe_float(metrics.get("dscr", 0.0))
    gap = _safe_float(sources_uses.get("gap", 0.0))
    ramp_months = _safe_int(inputs.get("ramp_months", 0), 0)

    items = []
    if annual_revenue > 0:
        items.append(f"Annual revenue needs to reach about ${annual_revenue:,.0f} for the base case to hold.")
    items.append(f"Labor needs to stay near {labor_pct * 100:,.1f}% of sales or better.")
    items.append(f"COGS needs to stay near {cogs_pct * 100:,.1f}% of sales or better.")
    if break_even_month:
        items.append(f"The business needs to get to cumulative cash break-even by about month {break_even_month}.")
    if dscr > 0:
        items.append(f"Debt service coverage needs to stay around {dscr:.2f}x or stronger.")
    if ramp_months > 0:
        items.append(f"The revenue ramp likely needs to stabilize within about {ramp_months} months.")
    if gap > 0:
        items.append("The remaining funding gap has to be closed before the structure is truly executable.")

    return items[:5]


def build_kill_criteria(inputs, metrics, downside_metrics, sources_uses):
    labor_pct = _safe_float(inputs.get("labor_pct", 0.0))
    cogs_pct = _safe_float(inputs.get("cogs_pct", 0.0))
    gap = _safe_float(sources_uses.get("gap", 0.0))
    break_even_month = downside_metrics.get("break_even_month")
    downside_lowest_cash = _safe_float(downside_metrics.get("lowest_cash", 0.0))

    items = []
    if gap > 0:
        items.append(f"Walk away or restructure if the funding gap remains at about ${gap:,.0f}.")
    items.append(f"Walk away or rework the deal if labor drifts materially above {(labor_pct + 0.05) * 100:,.1f}% of sales.")
    items.append(f"Walk away or rework the deal if COGS drifts materially above {(cogs_pct + 0.03) * 100:,.1f}% of sales.")
    if break_even_month is None or break_even_month > 18:
        items.append("Walk away if downside break-even is not visible within a reasonable period.")
    if downside_lowest_cash < 0:
        items.append("Walk away if a modest downside case creates a cash shortfall before the business stabilizes.")
    items.append("Walk away if major unanswered diligence items still remain when you are expected to commit.")
    return items[:5]


def build_downside_summary(metrics, downside_metrics):
    base_break_even = metrics.get("break_even_month")
    down_break_even = downside_metrics.get("break_even_month")
    base_roi = _safe_float(metrics.get("roi", 0.0))
    down_roi = _safe_float(downside_metrics.get("roi", 0.0))
    down_lowest_cash = _safe_float(downside_metrics.get("lowest_cash", 0.0))

    messages = []
    if base_break_even and down_break_even and down_break_even > base_break_even:
        messages.append(
            f"Break-even moves from about month {base_break_even} to month {down_break_even} under downside pressure."
        )
    if down_lowest_cash < 0:
        messages.append("The downside case creates a cash shortfall, which means the current structure has limited resilience.")
    if down_roi < base_roi:
        messages.append(
            f"Projected ROI falls from about {base_roi * 100:,.1f}% to {down_roi * 100:,.1f}% under modest stress."
        )
    if not messages:
        messages.append("The downside case does not show a single immediate break, but the deal still depends on disciplined execution.")
    return messages[:3]
